...
Require Import GFSubKernelUnionInter.
...
| RSAll : forall E L m1 m2 T1 T2 T3, wft E T1 ->
    ( forall X, X \notin L ->
                RS (E&(X !: m1 <>: T1)) m2 (T2 open_tt_var X) (T3 open_tt_var X)) -> 
    RS E m2 (typ_all m1 T1 T2) (typ_all m1 T1 T3)
| RSLeft  : forall A B C E m, RS E m A C -> wft E B -> RS E m (choose m A B) C
| RSRight : forall A B C E m, RS E m B C -> wft E A -> RS E m (choose m A B) C
| RSBoth  : forall A B C E m, RS E m A B -> RS E m A C -> RS E m A (choose m B C)
...
Lemma invSym : forall m A B E, RS E m A B -> RS E (flip m) B A.
  intros.
...
| R2All : forall E L m1 m2 T1 T2 T3 b, wft E T1 ->
    ( forall X, X \notin L ->
                R2 (E&(X !: m1 <>: T1)) true m2 (T2 open_tt_var X) (T3 open_tt_var X)) -> 
    R2 E b m2 (typ_all m1 T1 T2) (typ_all m1 T1 T3)
| R2Left  : forall A B C E m b, R2 E true m A C -> wft E B -> R2 E b m (choose m A B) C
| R2Right : forall A B C E m b, R2 E true m B C -> wft E A -> R2 E b m (choose m A B) C
| R2Both  : forall A B C E m b, R2 E true m A B -> R2 E true m A C -> R2 E b m A (choose m B C)
