(**************************************************************************************************
* Preservation and Progress for System-F kernel with duotyping, Union and Intersection types *
...
Fixpoint open_tt_rec (K : nat) (U : typ) (T : typ) {struct T} : typ :=
  ...
  | typ_or T1 T2    => typ_or (open_tt_rec K U T1) (open_tt_rec K U T2)
...
| SVarBnd : forall E X m T F,
    binds X (bind_tRel m T) E ->
    wft E T ->
    R E m T F ->
    R E m (typ_fvar X) F
| SVarBndFl :  forall E X m T F,
    binds X (bind_tRel m T) E ->
    wft E T -> R E (flip m) F T ->
    R E (flip m) F (typ_fvar X)
| SAll : forall E L m1 m2 T1 T2 T3,
    wft E T1 ->
    (forall X, X \notin L ->
               R (E&(X !: m1 <>: T1)) m2 (T2 open_tt_var X) (T3 open_tt_var X)) -> 
    R E m2 (typ_all m1 T1 T2) (typ_all m1 T1 T3)
| SLeft1  : forall A B C E m, R E m A C -> wft E B -> R E m (choose m A B) C
| SLeft2  : forall A B C E m, R E m A B -> wft E C -> R E m A (choose (flip m) B C)
| SRight1 : forall A B C E m, R E m B C -> wft E A -> R E m (choose m A B) C
| SRight2 : forall A B C E m, R E m A C -> wft E B -> R E m A (choose (flip m) B C)
| SBoth1  : forall A B C E m, R E m A B -> R E m A C -> R E m A (choose m B C)
| SBoth2  : forall A B C E m, R E m A C -> R E m B C -> R E m (choose (flip m) A B) C.
...
Fixpoint fv_tt (T : typ) {struct T} : vars :=
  ...
  | typ_and T1 T2 => (fv_tt T1) \u (fv_tt T2)
  | typ_or T1 T2 => (fv_tt T1) \u (fv_tt T2)
  | _           => \{}
  end.
...
Fixpoint subst_tt (Z : var) (U : typ) (T : typ) {struct T} : typ :=
  ...
  | typ_or T1 T2 => typ_or (subst_tt Z U T1) (subst_tt Z U T2)
  end.
...
Hint Resolve 
  STB1 STB2 SInt SFun SVarRefl SLeft1 SLeft2 SRight1 SRight2 SBoth1 SBoth2
...
Proof. introv Rel. split.
  induction* Rel. pick_fresh X. assert (X \notin L). auto.
  apply H1 in H2. apply okt_push_tvr_inv in H2. destruct* H2.
  induction* Rel; autos*.
  split. auto.
  destruct m. simpl. apply wft_top. apply wft_btm.
  split. destruct m. simpl. apply wft_btm. simpl. apply wft_top. auto.
  split. apply wft_all with (L:=L). auto. apply H1. apply wft_all with (L:=L). auto. apply H1.
  split. destruct m. simpl. apply wft_and. destruct IHRel. auto. auto.
  simpl. apply wft_or. autos*. auto. autos*.
  split. autos*.
  destruct m. simpl. apply wft_or. autos*. autos*.
  simpl. apply wft_and. autos*. autos*.
  split. destruct m. simpl. apply wft_and. autos*. autos*.
  simpl. apply wft_or. autos*. autos*. autos*.
  split. autos*. destruct m. simpl. apply wft_or. autos*. autos*.
  simpl. apply wft_and. autos*. autos*.
  split. autos*. destruct m. simpl. autos*. simpl. apply wft_or.
  autos*. autos*.
  split. destruct m. simpl. apply wft_or. autos*. autos*.
  simpl. apply wft_and. autos*. autos*. autos*.
Defined.
...
Lemma refl : forall E m A,
  ...
  - destruct m. apply SBoth1. apply SLeft1. eauto. eauto.
    apply SRight1. eauto. auto.
    apply SBoth2. apply SLeft2. auto. auto. apply SRight2. auto. auto.
  - destruct m. apply SBoth2. apply SLeft2. auto. auto. apply SRight2. auto. auto.
    apply SBoth1. apply SLeft1. auto. auto. apply SRight1. auto. auto.
Defined.
...
(*   apply SVarTrns with (X:=X); auto.  *)
  ...
  assert (E & F & G & X !: m1 <>: T1 = E&F&(G & X !: m1<>:T1)).
...
Proof.
  introv OK WF H. remember (mode_to_sub (flip m)) as T'. 
...
Proof. introv OK WF H. remember (mode_to_sub m) as T'. 
  induction H; try destruct m; inversion* HeqT'.
       - subst. eapply SVarBndFl. apply H. auto. simpl in *.
         apply IHR; eauto.
       - subst. eapply SVarBndFl. apply H. auto. simpl in *.
         apply IHR; eauto.
       - destruct m2; inversion H3.
...
Lemma Var_binds : forall E m C X,
  ...
  + unfold binds in *. rewrite H2 in H. injection H. intros.
    destruct m; simpl in *; inversion H4.
Defined.

Lemma choose_regular : forall E m T1 T2,
  wft E (choose m T1 T2) -> okt E -> wft E T1  /\ wft E T2.
Proof.
  introv rel. introv ok.
  split. destruct m; simpl in *. dependent induction rel; auto.
  dependent induction rel; auto.
  destruct m; simpl in *. dependent induction rel; auto.
  dependent induction rel; auto.
Defined.

Lemma invChoose : forall m A B C E, R E m A (choose m B C) -> R E m A B /\ R E m A C.
  intros m A B C E r.
  dependent induction r; eauto; try (destruct m; inversion x).
  - split.
    apply* STB2. apply choose_regular in H0. destruct* H0. auto.
    apply* STB2. apply choose_regular in H0. destruct* H0. auto.
  - destruct (IHr _ _ eq_refl). split; econstructor; eauto.
  - split. destruct m2; inversion x. destruct m2; inversion x.
...
Lemma invChoose2 : forall m A B C E, R E m (choose (flip m) A B) C -> R E m A C /\ R E m B C.
  intros.
  apply sym1 in H. apply invChoose in H. destruct H.
  apply sym1 in H. apply sym1 in H0.
  rewrite <- m_flip_inv in *.
  split; auto.
Defined.

Lemma sub_transitivity : forall E A B m, R E m A B -> forall C, R E m B C -> R E m A C.
  intros E A B m s.
  induction s; intros; simpl; try (destruct m; inversion H; fail); try (constructor; eauto; fail); auto.
  - dependent induction H; simpl; try (destruct m; inversion x); try (constructor; eauto; fail).
    + apply SVarBndFl with (T := T); eauto.
  - eapply SVarBnd; eauto.
  - apply IHs. apply sym1. eapply Var_binds; eauto. apply sym2 in H1. auto.
  - dependent induction H2; simpl; try (constructor; eauto; fail); try (destruct m; inversion x); auto.
    + apply STB1; eauto.  apply wft_all with (L:=L); eauto. intros. apply H0 in H4; auto.
    + apply SVarBndFl with (T := T); eauto.
    + apply SAll with (L := L \u L0); auto. 
  - apply invChoose2 in H0. destruct H0. apply IHs. auto.
  - apply invChoose2 in H0. destruct H0. apply IHs. auto.
...
Lemma sub_narrowing_aux_helper : forall P Q T F0 m1 m F E Z X,
  binds X (bind_tRel m T) (E & Z !: m1 <>: Q & F) ->
  wft (E & Z !: m1 <>: Q & F) T ->
  R (E & Z !: m1 <>: Q & F) m T F0 ->
  (forall m2 Q0 F1 E0 Z0,E & Z !: m1 <>: Q & F ~= E0 & Z0 !: m2 <>: Q0 & F1 ->
          R E0 m2 P Q0 -> R (E0 & Z0 !: m2 <>: P & F1) m T F0) ->
  R E m1 P Q ->
  R (E & Z !: m1 <>: P & F) m (typ_fvar X) F0.
Proof.
    intros P Q T F0 m1 m F E Z X H H0 SsubT IHSsubT PsubQ.
    tests EQ: (X = Z).
      ...
      asserts~ N: (ok (E & Z !:m1 <>: P & F)).
       ...
       replace E with (E&empty) in PsubQ; try apply concat_empty_r.
        ...
        rewrite concat_empty_r in PsubQ. rewrite concat_assoc in PsubQ.
         apply sub_regular in PsubQ. destruct* PsubQ. rewrite concat_empty_r.
         rewrite concat_assoc. apply* M. rewrite concat_empty_r in PsubQ. 
        apply sub_regular in PsubQ. destruct* PsubQ.  apply sub_transitivity with (B := Q).   
      assert (m = m1). {
        apply binds_middle_eq_inv in H. inversion* H. auto. }  
        subst.
...
Proof.
    ...
    apply SVarBndFl with (T:=P).
      asserts~ N: (ok (E & Z !:m1 <>: P & F)).
       ...
       replace E with (E&empty) in PsubQ; try apply concat_empty_r.
        ...
        apply sub_transitivity with (B := Q).
      assert (m = m1). {
        apply binds_middle_eq_inv in H. inversion* H. auto. }  
        subst. binds_get H. autos*. 
       assert (m = m1). {
        apply binds_middle_eq_inv in H. inversion* H. auto. }
      subst.
        do_rew* concat_assoc (apply_empty* sub_weakening).
        inversion H. binds_get H. apply sym1. auto.
  ...
  apply SLeft1. eauto. eauto.
  apply SLeft2. eauto. eauto.
  apply SRight1. eauto. eauto.
  apply SRight2. eauto. eauto.
  apply SBoth1. eauto. eauto.
  apply SBoth2. eauto. eauto.
Defined.
...
Proof.
  ...
  apply SAll with (L:=L \u \{Z}).
   apply* wft_subst_tb. apply sub_regular in PsubQ. destruct* PsubQ. 
  ...
  destruct* H4. lets M:PsubQ. apply sub_regular in PsubQ. destruct* PsubQ. 
   unsimpl (subst_tb Z P (bind_tRel m0 T1)).
   intros. rewrite* subst_tt_open_tt_var. rewrite* subst_tt_open_tt_var. 
   apply_ih_map_bind* H1.
  destruct m. simpl. apply SLeft1. eauto. eauto.
  simpl. apply SLeft1. eauto. eauto.
  destruct m. simpl. apply SLeft2. autos*. eauto.
  simpl. apply SLeft2. autos*. eauto.
  destruct m. simpl. apply SRight1. autos*. eauto.
  simpl. apply SRight1. autos*. eauto.
  destruct m. simpl. apply SRight2. autos*. eauto.
  simpl. apply SRight2. autos*. eauto.
  destruct m. simpl. apply SBoth1. autos*. eauto.
  simpl. apply SBoth1. autos*. eauto.
  destruct m. simpl. apply SBoth2. autos*. eauto.
  simpl. apply SBoth2. autos*. eauto.
Defined.
...
- destruct m; inversion x;split;auto;auto.
