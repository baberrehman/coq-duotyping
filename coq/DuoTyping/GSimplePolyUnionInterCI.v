(* System F + Top + Bottom + Unions + Intersections + Generalized subtyping 
Version with a customized induction principle
*)
...
Hint Constructors type term wft ok okt value red R.
...
Lemma sym1 : forall A B m, R m A B -> R (flip m) B A.
  ...
  - destruct m; simpl in *; constructor; auto.
...
Lemma indR : 
    forall (P : Mode -> typ -> typ -> Prop),
      ...
      (forall (m : Mode) X, P m (typ_fvar X) (typ_fvar X)) ->
      (forall (L : fset var) (m : Mode) (T1 T2 : typ),
        (forall X : var, X \notin L -> R m (T1 open_tt_var X) (T2 open_tt_var X)) ->
        (forall X : var, X \notin L -> P m (T1 open_tt_var X) (T2 open_tt_var X)) ->
        P m (typ_all T1) (typ_all T2)) ->
      (forall (A B C : typ) (m : Mode), R m A C -> P m A C -> P m (choose m A B) C) ->
      (forall (A B C : typ) (m : Mode), R m B C -> P m B C -> P m (choose m A B) C) ->
       (forall (A B C : typ) (m : Mode),
        R m A B -> P m A B -> R m A C -> P m A C -> P m A (choose m B C)) ->
  ...
  intros.
  induction H8; eauto.
  - apply H7. constructor. apply sym1. auto.
    apply H4. apply sym1; auto. apply H7; rewrite flip_flip; auto.
  - apply H7. apply SRight1. apply sym1. auto.
    apply H5. apply sym1; auto. apply H7; rewrite flip_flip; auto.
  - apply H7. apply SBoth1. apply sym1. auto. apply sym1. auto. 
    apply H6. apply sym1; auto. apply H7; rewrite flip_flip; auto.
    apply sym1; auto. apply H7; rewrite flip_flip; auto.
Defined.
...
Lemma refl : forall (*E*) m A, type A (*wft E A*) -> R m A A.
  intros.
  induction H; eauto.
  - destruct m. apply STB1. apply STB2.
  - destruct m. apply STB2. apply STB1.
  - apply SFun; auto. apply sym1. auto.
Defined.
...
Lemma invChoose2 : forall m A B C, R m (choose (flip m) A B) C -> R m A C /\ R m B C.
  ...
  rewrite flip_flip in H. rewrite flip_flip in H0.
  split; auto.
Defined.
...
Lemma trans : forall A B m, R m A B -> forall C, R m B C -> R m A C.
  ...
  - dependent induction H; simpl in *; auto; try (destruct m; inversion x).
    apply SLeft2. eapply IHR; eauto.
    apply SRight2. eapply IHR; eauto.
    apply SBoth1; eauto.
  - dependent induction H1; simpl in *; auto; try (destruct m; inversion x).
    + apply SAll with (L := L \u L0); intros; auto.
    +  apply SLeft2. eapply IHR; eauto.
    + apply SRight2. eapply IHR; eauto.
    + apply SBoth1; eauto.
  - apply invChoose2 in H. destruct H. apply IHs. auto.
  - apply invChoose2 in H. destruct H. apply IHs. auto.
  - dependent induction H; simpl in *; auto; try (destruct m; inversion x; fail).
...
Proof.
  introv SsubT PWFT.
  induction SsubT using indR; subst; simpl subst_tt.
  - apply* SInt.
  - destruct m; apply* STB1.
  - apply* SFun.
  - case_var.
      apply* refl.
      apply wft_type in PWFT. auto.
      apply* SVarRefl.
  ...
  - destruct m. simpl. apply* SLeft1. simpl. apply* SLeft1.
  - destruct m. simpl. apply* SRight1. simpl. apply* SRight1.
  - destruct m. simpl. apply* SBoth1. simpl. apply* SBoth1.
  - apply sym2. auto.
Qed. 
...
Proof.
  ...
  destruct~ (typing_inv_abs Typ1 (U1:=T1) (U2:=T2)) as [P1 [S2 [L P2]]].
    ...
    destruct H0. apply wft_type in H2. auto.
    pick_fresh X. (*apply wft_type in H2. auto. pick_fresh X.*) forwards~ K: (P2 X). destruct K.
  ...
  - inversions Red; try solve [ apply* typing_tapp ].
   ...
   apply refl. (*with (E:=E)*) destruct Typ. destruct H1. apply wft_type in H3. auto.
...
{ rewrite st. 
    ...
    destruct Typ. destruct H0. apply wft_type in H1. auto. }
...
Proof. 
  ...
  apply refl (*with (E:=empty)*); auto. subst. apply typing_regular in Typ. destruct Typ.
      destruct H0. apply wft_type in H1. auto. }
  clear st. gen_eq E: (@empty typ).
  induction Typ; introv EQE; 
   try solve [ inversion Val | inversion EQE | eauto | inversion H ].
