(**************************************************************************************************
* Preservation and Progress for System-F kernel with Subtyping, Union and Intersection types        *
...
Inductive sub : env -> typ -> typ -> Prop :=
  ...
  | sub_all : forall L E T1 T2 T3,
      (forall X, X \notin L ->
          sub (E & X ~<: T1) (T2 open_tt_var X) (T3 open_tt_var X)) ->
      sub E (typ_all T1 T2) (typ_all T1 T3)
  | sub_and1  : forall E t t1 t2,
        sub E t t1 -> sub E t t2 -> sub E t (typ_and  t1 t2) 
  | sub_and2  : forall E t t1 t2 ,
        sub E t1 t -> wft E t2 -> sub E (typ_and t1 t2) t 
  | sub_and3  : forall E t t1 t2,
        sub E t2 t -> wft E t1 -> sub E (typ_and t1 t2) t 
  | sub_or1   : forall E t t1 t2,
        sub E t t2 -> sub E t1 t2 -> sub E (typ_or t t1) t2
  | sub_or2   : forall E t t1 t2,
        sub E t t1 -> wft E t2 -> sub E t (typ_or t1 t2)
  | sub_or3   : forall E t t1 t2,
        sub E t t2 -> wft E t1 -> sub E t (typ_or t1 t2).
...
Proof.
  ...
  apply* (@wft_var U0). binds_cases H; try discriminate; autos*.
  apply_fresh* wft_all as Y. apply_ih_bind* H1.
Qed.
...
Lemma okt_push_inv : forall E X B,
  okt (E & X ~ B) -> (exists T, B = bind_sub T) \/ exists T, B = bind_typ T.
Proof.
  introv O. inverts O.
...
Lemma okt_push_tvr_inv : forall E X T,
 ...
 introv O. induction F using env_ind.
  ...
  rewrite concat_assoc in *.
   lets: okt_push_inv O. destruct H; subst.
     destruct H. subst.
     lets (?&?): (okt_push_tvr_inv O).
      applys~ okt_sub. applys* wft_strengthen. destruct* H0.
...
Proof.
 introv O W. induction F using env_ind.
  ...
  rewrite map_push. rewrite concat_assoc in *.
   ...
   lets (?&?): (okt_push_tvr_inv O).
      applys~ okt_sub. destruct* H0. autos*.
      lets (?&?&?): (okt_push_typ_inv O).
...
Proof.
  ...
  split.
  pick_fresh X.
  assert (X \notin L). eauto. destruct (H0 X H1).
    apply okt_push_tvr_inv in H2. autos*.
  split; apply_fresh* wft_all as Y.
  pick_fresh X.
  assert (X \notin L). eauto. destruct (H0 X H1).
    apply okt_push_tvr_inv in H2. autos*.
    destruct~ (H0 Y); apply_empty* (@wft_narrow T1).
  pick_fresh X.
  assert (X \notin L). eauto. destruct (H0 X H1).
    apply okt_push_tvr_inv in H2. autos*.
    destruct~ (H0 Y); apply_empty* (@wft_narrow T1).
...
Proof.
  induction 1; splits*.
   ...
   apply_fresh* term_abs as y.
     pick_fresh y. specializes H0 y. destructs~ H0.
      forwards*: okt_push_typ_inv. destruct H3.
      destruct H4. apply wft_type in H4. auto.
  ...
  destruct IHtyping. destruct H2. inversion H3.  subst.
   applys* wft_open T1. apply sub_regular in H0. destruct* H0.
   apply sub_regular in H0. destruct* H0.
Qed. 
...
Lemma sub_reflexivity : forall E T,
  ...
  sub E T T .
Proof.
  introv Ok WFT. lets W:(wft_type WFT). gen E.
  induction W; intros; inversions WFT; eauto.
  - apply_fresh* sub_all as Y.
  - apply sub_and1. apply sub_and2. eauto. eauto.
    apply sub_and3. eauto. eauto.
  - apply sub_or1. apply sub_or2. eauto. eauto.
    apply sub_or3. eauto. eauto.
Qed.
...
Proof.
  ...
  pick_fresh X.
  assert (X \notin L). eauto.
  eapply sub_regular in H; eauto. autos*.
  (* Cases: Union and Intersection *)
  apply sub_and1; eauto.
  apply sub_and2; eauto.
  apply sub_and3; eauto.
  apply sub_or1; eauto.
  apply sub_or2; eauto.
  apply sub_or3; eauto.
Qed.
...
Proof.
  ...
  gen_eq G: (E & Z ~<: Q & F). gen F.
  induction SsubT; introv EQ; subst.
  apply* sub_top.
  apply* sub_refl_tvar.
  tests EQ: (X = Z); subst.
    apply (@sub_trans_tvar P); autos* (@okt_narrow Q).
  ...
  apply* sub_arrow.
  apply_fresh* sub_all as Y. apply_ih_bind* H0.
  apply sub_and1. eauto. eauto.
  apply sub_and2. eauto. eauto.
  apply sub_and3. eauto. eauto.
  apply sub_or1. eauto. eauto.
  apply sub_or2. eauto. eauto.
  apply sub_or3. eauto. eauto.
Qed.

Lemma invAndS1 : forall E t t1 t2, sub E t (typ_and t1 t2) -> sub E t t1 /\ sub E t t2.
Proof.
  ...
  - auto.
  - destruct (IHsub _ _ eq_refl).
    split; apply sub_and2; eauto.
  - destruct (IHsub _ _ eq_refl).
    split; apply sub_and3; eauto.
  - destruct (IHsub1 _ _ eq_refl).
    destruct (IHsub2 _ _ eq_refl).
    split; constructor; auto.
Defined.

Lemma invOrS1 : forall E t t1 t2, sub E (typ_or t1 t2) t -> sub E t1 t /\ sub E t2 t.
Proof.
  intros. dependent induction H.
  - inversion H0. auto.
  - destruct (IHsub1 _ _ eq_refl);
...
Hint Resolve invAndS1 invOrS1 sub_trans_tvar sub_and1
sub_and2 sub_and3 sub_or1 sub_or2 sub_or3.
...
Proof.
  ...
  induction W; intros.
  - dependent induction QsubT; try discriminate; eauto.
  - dependent induction SsubQ; try discriminate; eauto.
  - dependent induction QsubT; try discriminate; eauto.
    clear IHQsubT1. clear IHQsubT2.
    dependent induction SsubQ; eauto.
  - dependent induction SsubQ; try discriminate; eauto.
    dependent induction QsubT; eauto 4 using sub_trans_tvar.
      (*sub_all / sub_top*)
    assert (sub E (typ_all T1 T3) (typ_all T1 T2)).
      (*proof of assertion*)
      pick_fresh y; apply* sub_all.
    auto.
    (*sub_all / sub_all*)
  apply_fresh sub_all as X; autos*.
  lapply (H0 X); [ intros K | auto ].
  apply (K (T2 open_tt_var X)); auto.
  - subst. dependent induction QsubT. auto. eauto.
    clear IHQsubT.
    dependent induction SsubQ; eauto.
    apply invAndS1 in SsubQ. destruct SsubQ. eauto. eauto. eauto.
  - subst. dependent induction SsubQ.
    eauto. eauto. eauto. eauto.
     clear IHSsubQ.
     dependent induction QsubT; eauto.
    apply invOrS1 in QsubT. destruct QsubT. eauto.
Qed.
...
Proof.
  ...
  gen_eq G: (E & x ~: U & F). gen F.
  induction SsubT; introv EQ; subst; autos* wft_strengthen.
  (* case: fvar trans *)
...
Proof.
  introv TypT TypU. gen_eq E': (E & x ~: U & F). gen F.
...
Proof.
  introv Typ PsubQ. gen_eq G: (E & Z ~<: Q & F). gen F.
  induction Typ; introv EQ; subst; simpls subst_tt; simpls subst_te.
...
Proof.
  ...
  pick_fresh X.
  assert (X \notin L ). eauto.
  eapply typing_regular in H; eauto.
  destruct H. apply okt_strengthen_l in H. auto.
  pick_fresh X.
  assert (X \notin L). eauto.
  eapply typing_regular in H; eauto.
  destruct H. destruct H2. autos*.
   exists T1. let L1 := gather_vars in exists L1.
   intros Y Fr. splits. 
    apply_empty* (@typing_narrowing U1).
  apply sub_reflexivity.
  pick_fresh X.
  assert (X \notin L ). eauto.
  eapply typing_regular in H; eauto.
...
Proof.
  ...
  destruct~ (typing_inv_tabs Typ (U1:=T1) (U2:=T2)) as [P1 [S2 [L P2]]].
    ...
    pick_fresh X. forwards~ K: (P2 X). destruct K.
     ...
     replace E with (E & map (subst_tb X T) empty).
     apply* (@typing_through_subst_te T1).
    assert (typing (E & X ~<: T1) (e0 open_te_var X) (T2 open_tt_var X)).
   eapply typing_sub; eauto.
   replace (E & X ~<: T1 & empty) with (E & X ~<: T1); auto.
...
Proof.
  ...
  gen_eq T: (typ_arrow U1 U2). intro st.
   assert (sub empty T (typ_arrow U1 U2)). 
{ rewrite st. 
    apply sub_reflexivity with (E:=empty); auto. subst. apply typing_regular in Typ. 
    destruct Typ. destruct H0. auto. }
  clear st. gen_eq E: (@empty typ).  gen U1 U2.
  induction Typ; introv EQT EQE; 
   try solve [ inversion Val | inversion EQT | eauto ].
    subst. assert (sub E0 S (typ_arrow U1 U2)). { 
    eapply sub_transitivity. apply H. apply EQT. }
   eapply IHTyp. apply Val. apply H0. reflexivity.
Qed.

Lemma tall_inv : forall E T1 T2 M N,
sub E (typ_all T1 M) (typ_all T2 N) -> T1=T2.
Proof. intros.
- inversion H. subst. autos*.
Qed.

Lemma canonical_form_tabs : forall t T U,
  value t -> typing empty t (typ_all T U) -> 
  exists e1, t = trm_tabs T e1.
Proof.
  introv Val Typ. 
  gen_eq Ty: (typ_all T U). intro st. 
  assert (sub empty Ty (typ_all T U)). { rewrite* st.
  apply sub_reflexivity with (E:=empty); auto. subst. apply typing_regular in Typ. destruct Typ. 
   destruct H0. auto. }
  clear st. gen_eq E: (@empty typ).
  induction Typ; introv EQE; 
   try solve [ inversion Val | inversion EQE | eauto | inversion H ].
  exists e1. lets Q:(tall_inv H). destruct Q. subst*.
  subst. apply* IHTyp. eapply sub_transitivity. apply H0. apply H.
Qed.
...
Proof.
  ...
  right. destruct* IHTyp as [Val1 | [e1' Rede1']]. 
    destruct (canonical_form_tabs Val1 Typ).
      subst. exists* (open_te x T). 
  (* case: sub *)
  autos*.
Qed.