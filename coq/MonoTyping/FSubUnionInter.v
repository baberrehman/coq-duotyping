...
Hint Resolve sub_top sub_refl_tvar sub_trans_tvar sub_arrow sub_all
              sub_and1 sub_and2 sub_and3 sub_or1 sub_or2 sub_or3.
...
Proof.
  ...
  split. autos*. split;
   apply_fresh* wft_all as Y.
    destruct~ (H1 Y); apply_empty* (@wft_narrow T1).
    destruct~ (H1 Y); apply_empty* (@wft_narrow T1).
...
Hint Resolve invAndS1 invOrS1 sub_and1
...
sub E (typ_all T1 M) (typ_all T2 N) -> sub E T2 T1.
Proof. intros.
...
Lemma canonical_form_tabs : forall e U1 U2,
  ...
  exists V, exists e1, e = trm_tabs V e1.
Proof.
...
intros U1 U2 EQT EQE. subst.
    try solve [ inversion Val | inversion EQT | eauto ].
  (*assert(typing empty e (typ_all U1 U2)).
  eapply typing_sub; eauto.*)

  (*typing_sub*)
    inversion H; subst; eauto.
    inversion H0.
    assert (get X empty = @None bind).
    apply get_empty.
    rewrite H2 in H3.
    inversion H3.
